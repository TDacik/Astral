...
let print_term print_var t = match SL.Term.view t with
  | Var v -> print_var v

let print_atom p phi =
  let var = print_term p.print_var in
  match SL.view phi with
  | Eq [x1; x2] -> (var x1) ++ p.eq ++ (var x2)
  | Distinct [x1; x2] -> (var x1) ++ p.eq ++ (var x2)
  | PointsTo (x, def, ys) ->
...
let print_symbolic_heap ?(precise=true) ?(p=default_printer) phi =
  ...
  else failwith "Not a symbolic heap"
