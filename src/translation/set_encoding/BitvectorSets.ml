...
let compute_width set_sort = Sort.cardinality @@ Sort.get_dom_sort set_sort
...
let mk_enumeration sort elems =
  let width = Sort.get_width sort in
  Bitvector.mk_or width (List.map mk_singleton elems)
...
let mk_disjoint set1 set2 =
  ...
  let bv_and = Bitvector.mk_and width [set1; set2] in
  mk_eq [bv_and; Bitvector.mk_zero width]
...
let mk_union sets sort = Bitvector.mk_or (Sort.get_width sort) sets
let mk_inter sets sort = Bitvector.mk_and (Sort.get_width sort) sets
...
let hex_to_bin str = BatString.fold_left (fun acc digit ->
  ...
  | 'f' -> "1111"
  | 'x' | '#' -> ""
  | other -> failwith (Format.asprintf "Unexpected hex digit: %c" other)
  in
  acc ^ bits
) "" str
...
let inverse_translation (bitstring : String.t) n =
  let acc = Sets.mk_empty (Sort.Bitvector n) in
  let set = match String.get bitstring 1 with
  | 'b' ->
    BatString.fold_lefti
      (fun acc index bit ->
        if bit == '1'
        then Bitvector.mk_const_of_int index n :: acc
        else acc
  ...
  in
  Sets.mk_constant (Sort.Bitvector n) set
...
let rec rewrite (t : SMT.t) =
  let rewrite_var var =
    let name, sort = SMT.Variable.describe var in
    if Sort.is_set sort then SMT.Variable.mk name (Sort.get_dom_sort sort)
    else var
  in
  SMT.map_view
    (function
