...
let rec push_star_aux xs psis = match xs with
  ...
  | x :: xs ->
    ...
    | plus, minus ->
      ...
      SSL.mk_star [push @@ SSL.mk_exists [x] phi_plus; phi_minus]
...
and apply phi = match phi with
  ...
  | SSL.Forall (xs, And (psi1, psi2)) ->
    SSL.mk_and [push @@ SSL.mk_forall xs psi1; push @@ SSL.mk_forall xs psi2]

  (* Exists over disjunction *)
  | SSL.Exists (xs, Or (psi1, psi2)) ->
    SSL.mk_or [push @@ SSL.mk_exists xs psi1; push @@ SSL.mk_exists xs psi2]
  ...
  | SSL.Forall (xs, Star (psi1, psi2)) ->
    SSL.mk_star [push @@ SSL.mk_forall xs psi1; push @@ SSL.mk_forall xs psi2]
  *)

  | SSL.And (psi1, psi2) -> SSL.mk_and [push psi1; push psi2]
  | SSL.Or (psi1, psi2) -> SSL.mk_or [push psi1; push psi2]
  | SSL.Not psi -> SSL.mk_not (push psi)
  | SSL.GuardedNeg (psi1, psi2) -> SSL.mk_gneg (push psi1) (push psi2)
  | SSL.Star psis -> SSL.mk_star @@ List.map push psis
  | SSL.Septraction (psi1, psi2) -> SSL.mk_septraction (push psi1) (push psi2)
  | SSL.Forall (xs, psi) -> SSL.mk_forall xs (push psi)
  | SSL.Exists (xs, psi) -> SSL.mk_exists xs (push psi)
