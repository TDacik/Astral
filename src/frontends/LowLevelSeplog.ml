...
module Context = struct
  ...
  let init width =
    let loc_sort = Sort.mk_bitvector width in
    let arr_sort = Sort.mk_array loc_sort loc_sort in
    {
      width = width;
      null = SMT.Bitvector.mk_const_of_int 0 width;
...
let pto_valid_axiom ctx = function
  ...
  | Range (start, size) ->
    let sum = SMT.Bitvector.mk_plus ctx.width [start; size] in
    SMT.Boolean.mk_and [
...
let pto_disjoint_axiom ctx (x, y) = match x, y with
  ...
  | Singleton t, Range (start, size) | Range (start, size), Singleton t ->
    let max = SMT.Bitvector.mk_plus ctx.width [start; size] in
    SMT.Boolean.mk_or [
      SMT.Bitvector.mk_lesser t start;
      SMT.Bitvector.mk_lesser_eq max t;
    ]
  | Range (start1, size1), Range (start2, size2) ->
    let max1 = SMT.Bitvector.mk_plus ctx.width [start1; size1] in
    let max2 = SMT.Bitvector.mk_plus ctx.width [start2; size2] in
    SMT.Boolean.mk_or [
...
let check_sat ~address_width phi =
  let module Backend = BitwuzlaNative.Init () in
  let ctx = Context.init address_width in
