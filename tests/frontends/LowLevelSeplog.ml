...
module LL = LowLevelSeplog

let x = LL.Term.mk_var 16 "x"
let y = LL.Term.mk_var 16 "y"
...
let test_check_sat phi =
  ...
  Alcotest.check' Alcotest.bool ~msg ~actual:(LL.check_sat ~address_width:16 phi = `Sat) ~expected:true

let test_check_unsat phi =
  let msg = Format.asprintf "UNSAT: %s" (LL.show phi) in
  Alcotest.check' Alcotest.bool ~msg ~actual:(LL.check_sat ~address_width:16 phi = `Sat) ~expected:false
